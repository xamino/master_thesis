...
While this could be done via a central server, nowadays the common solution to the problem is a distributed hash table~\cite{web:site:wiki:dht} which is used to look up the required peers.
...
\begin{tabulary}{\textwidth}{R || C | C | C | C }
      & Peer to Peer & Server to Client & Open Source & Secure Storage \\
    \hline \hline
    Dropbox &  & \checkmark &  &  \\
    \hline
    OneDrive &  & \checkmark & & \\
    ...
    \hline
    BitTorrent Sync & \checkmark &  &  &  \\
    \hline
    Syncthing & \checkmark &  & \checkmark &  \\
\end{tabulary}
\caption[TODO]{TODO}
\label{table:software_comparison}
\end{table}

TODO: the table has to be reworked, made more expansive.
