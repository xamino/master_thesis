...
TODO: signal lock, lock acceptance, lock denial, and lock freeing.

\subsubsection{Request Operation}
\label{subs:Fetch Operation}

TODO: note difference to fetch operation above.
Most important that this operation can request the encrypted model of the encrypted peer.

\subsubsection{Put Operation}
\label{subs:Put Operation}

TODO: command to tell the other peer to receive the following file transfer.
Places new model and object plus deltas on the encrypted peer, ready for other peers to find later.
...
TODO: ONCE THE ABOVE HAVE BEEN DONE I WILL NEED TO ADD SHORT HINTS TO HOW CONLFICTS ARE RESOLVED FROM THE VIEW POINT OF THE ENCRYPTED PEERS (MEANING BY HAVING TRUSTED PEERS RESOVLE THEM FOR US AND JUST TELLING US IN WHAT STATE TO PLACE THE ENCRYPTED FILES NOW).
...
%TODO: may have to put some further work into securing updates between trusted peers via encrypted peers.
...
TODO: change the following, encrypted peers have just become simple data dumps!
...
Without checks this can be used to introduce bogus updates into the system: random data can be used to overwrite valid files with the modify operation.
Furthermore create, remove, and move updates can be initiated without the user's consent.
...
As the hash is also encrypted before file transfer, decryption and comparison to the decrypted file will flag invalid updates.
It is therefore theoretically impossible to overwrite valid data by having access to an encrypted peer.

The second case is much more difficult to catch: the illegal introduction of create, move, and remove updates.
We propose to do this via a signing these transactions.
For this we take the operation name plus the file hash and sign them using a public private key pair which we also store in the \textit{auth.json} file.

TODO: Houston, we have a problem.
One: encrypted peers must be able to merge.
Two: encrypted peers are contrary however not to be allowed to introduce changes...
Solution: encrypted peers are dumb data dumps.
They only retain the last known version of model and files, all encrypted(!).
Trusted peers can lock them, synchronize, tell them of differences, and once in sync, re-lock them.
