...
\begin{figure}[htp]
    ...
    \end{lstlisting}
\caption[Request Message]{Message used to request objects from an encrypted peer. Note that the object attribute isn't including if requesting the model.}
\label{json:request_message}
\end{figure}

Figure~\ref{json:request_message} highlights the message used to get the encrypted peer to initiate a file transfer of the requested file or the encrypted model.
Unlike the fetch message seen in section~\ref{subs:Fetch File Message} this operation is explicitely different as it will only result in encrypted files being sent.
Furthermore the option to request the model from the other peer as an explicit action further differentiates the two operations.
